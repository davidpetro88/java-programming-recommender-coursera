 ...
 */
public class FirstEfficientRater {

    public FirstEfficientRater() {}
    ...
    public ArrayList<EfficientRater> loadRaters(String filename) {
        ...
        fileResource.getCSVParser().forEach(record -> {
            ...
            Optional<EfficientRater> raterIdExist = raterList.stream().filter( filterRater -> filterRater.getID().equals(rater_id)).findFirst();            
	...
	private EfficientRater addRater(String rater_id, String movie_id, Double rating) {
		EfficientRater currentRater = new EfficientRater(rater_id);
    ...
    public void testLoadRaters() {
        ...
        String id = "2";
        Stream<EfficientRater> filter = raterList.stream().filter(r -> r.getID().equals(id));
        filter.forEach(rate -> {
        ...
        Optional<EfficientRater> findFirst = raterList.stream().filter(r -> r.numRatings() == maxRatings.getAsInt()).findFirst();
